...
export async function generateMessageSuggestions(
  ...
  if (import.meta.env.VITE_OPENAI_API_KEY) {
    try {
      return await generateAISuggestions(contact, energy);
    } catch (error) {
  ...
  return generateTemplateSuggestions(contact, energy);
}
...
async function generateAISuggestions(
  ...
  const prompt = buildPrompt(contact, energy);
...
function generateTemplateSuggestions(
  ...
  while (suggestions.length < 3) {
    suggestions.push({
      text: `Hey ${name}! How are things going? Would love to catch up soon.`,
      tone: 'casual'
    });
  }
...
function buildPrompt(contact: Contact, energy: EnergyLevel): string {
  const name = contact.preferred_name || contact.first_name;
  const context = [
    `Name: ${name}`,
